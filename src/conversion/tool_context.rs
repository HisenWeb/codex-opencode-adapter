...
impl ToolContext {
    ...
    pub fn chat_name_for_response_function(&self, name: &str, namespace: Option<&str>) -> String {
        if let Some(namespace) = namespace.filter(|value| !value.is_empty()) {
            ...
            return format!("{namespace}__{name}")
                .chars()
    ...
    fn add_namespace(&mut self, item: &Value) {
        ...
        for child in children {
                ...
                .filter(|s| !s.trim().is_empty())
            {
                self.add_function(
                    source,
                    Some(format!("{namespace}__{child_name}")),
    ...
    fn add_function(
        ...
        let original = original_name.unwrap_or_else(|| {
            source
                .get("name")
                .and_then(Value::as_str)
                .unwrap_or("")
        ...
        if let Some(namespace) = namespace {
            self.namespace_name_to_chat_name
                .insert((namespace, spec.name.clone()), chat_name.clone());
        }
        self.add_chat_tool(
            original.as_str(),
            spec,
    ...
    fn add_chat_tool(
        ...
        self.reverse_names
            .insert(spec.chat_name.clone(), original.to_string());
        self.specs_by_chat_name
            .insert(spec.chat_name.clone(), spec.clone());
