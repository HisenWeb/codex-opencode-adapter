...
impl StreamAssembler {
    ...
    pub fn accept(&mut self, chunk: &Value) -> anyhow::Result<()> {
        if let Some(usage) = chunk.get("usage").filter(|v| !v.is_null()) { self.usage = usage.clone(); }
        let Some(choices) = chunk.get("choices").and_then(Value::as_array) else { return Ok(()); };
        for choice in choices {
            let delta = choice.get("delta").unwrap_or(&Value::Null);
            if let Some(text) = reasoning_text(delta) { self.push_reasoning_delta(&text)?; }
            ...
            if let Some(reason) = choice.get("finish_reason").and_then(Value::as_str) { self.finish_reason = Some(reason.to_string()); }
        }
        Ok(())
    }
    ...
    pub fn finalize(&mut self) -> anyhow::Result<Value> {
        ...
        let mut assistant = json!({"role":"assistant","content":self.content});
        ...
        for key in keys {
            ...
            let output_index = call.get("output_index").and_then(Value::as_u64).unwrap_or_else(|| self.allocate_output_index() as u64) as u32;
            ...
            let item = json!({"type":"function_call","id":item_id,"call_id":call_id,"name":self.reverse_names.get(&raw_name).cloned().unwrap_or(raw_name),"arguments":arguments,"status":"completed"});
            ...
            self.emit_event("response.output_item.done", json!({"type":"response.output_item.done","output_index":output_index,"item":item}))?;
            output.push(item);
        }
        ...
        (self.state_put)(StoredResponse {
            ...
            messages: repair_history(&self.base_messages, None)?.into_iter().chain(std::iter::once(assistant)).collect(),
        ...
        let (status, incomplete) = completion_status(&self.content, &pending, self.finish_reason.as_ref().map(|s| &Value::String(s.clone())));
        ...
        self.emit_event(event, json!({"type":event,"response":response}))?;
    ...
    pub fn fail(&mut self, error_type: &str, message: &str) -> anyhow::Result<Value> {
        ...
        self.emit_event("response.failed", json!({"type":"response.failed","response":response}))?;
        self.terminal_emitted = true;
        Ok(response)
    }
    ...
    fn push_reasoning_delta(&mut self, delta: &str) -> anyhow::Result<()> {
        if self.reasoning_output_index.is_none() {
            ...
            self.emit_event("response.output_item.added", json!({"type":"response.output_item.added","output_index":index,"item":{"id":self.reasoning_item_id,"type":"reasoning","status":"in_progress","summary":[]}}))?;
            self.emit_event("response.reasoning_summary_part.added", json!({"type":"response.reasoning_summary_part.added","item_id":self.reasoning_item_id,"output_index":index,"summary_index":0,"part":{"type":"summary_text","text":""}}))?;
        }
        self.reasoning.push_str(delta);
        self.emit_event("response.reasoning_summary_text.delta", json!({"type":"response.reasoning_summary_text.delta","item_id":self.reasoning_item_id,"output_index":self.reasoning_output_index,"summary_index":0,"delta":delta}))
    }
    ...
    fn finish_reasoning_item(&mut self) -> anyhow::Result<()> {
        ...
        self.emit_event("response.reasoning_summary_text.done", json!({"type":"response.reasoning_summary_text.done","item_id":self.reasoning_item_id,"output_index":index,"summary_index":0,"text":self.reasoning}))?;
        self.emit_event("response.reasoning_summary_part.done", json!({"type":"response.reasoning_summary_part.done","item_id":self.reasoning_item_id,"output_index":index,"summary_index":0,"part":{"type":"summary_text","text":self.reasoning}}))?;
    ...
    fn push_text_delta(&mut self, text: &str) -> anyhow::Result<()> {
        ...
        self.emit_event("response.output_text.delta", json!({"type":"response.output_text.delta","output_index":self.text_output_index,"content_index":0,"item_id":self.message_item_id,"delta":text}))
    }
    ...
    fn ensure_text_started(&mut self) -> anyhow::Result<()> {
        ...
        self.emit_event("response.output_item.added", json!({"type":"response.output_item.added","output_index":index,"item":{"type":"message","id":self.message_item_id,"status":"in_progress","role":"assistant","content":[]}}))?;
        self.emit_event("response.content_part.added", json!({"type":"response.content_part.added","output_index":index,"content_index":0,"item_id":self.message_item_id,"part":{"type":"output_text","text":"","annotations":[]}}))
    }
    ...
    fn finish_text_item(&mut self) -> anyhow::Result<()> {
        ...
        self.emit_event("response.output_text.done", json!({"type":"response.output_text.done","output_index":index,"content_index":0,"item_id":self.message_item_id,"text":self.content}))?;
        self.emit_event("response.content_part.done", json!({"type":"response.content_part.done","output_index":index,"content_index":0,"item_id":self.message_item_id,"part":{"type":"output_text","text":self.content,"annotations":[]}}))?;
        self.emit_event("response.output_item.done", json!({"type":"response.output_item.done","output_index":index,"item":item}))?;
        self.text_done = true;
        Ok(())
    }
    ...
    fn accept_tool_delta(&mut self, delta: &Value) -> anyhow::Result<()> {
        ...
        let entry = self.tool_calls.entry(index).or_insert_with(|| json!({
            "id": delta.get("id").and_then(Value::as_str).map(ToString::to_string).unwrap_or_else(|| format!("call_{}", Uuid::new_v4().simple())),
            "item_id": format!("fc_{}", Uuid::new_v4().simple()),
            "name":"",
            "arguments":"",
            "added":false,
        }));
        if let Some(id) = delta.get("id").and_then(Value::as_str) { entry["id"] = Value::String(id.to_string()); }
        let function = delta.get("function").unwrap_or(&Value::Null);
        if let Some(name) = function.get("name").or_else(|| delta.get("name")).and_then(Value::as_str) {
        ...
        if let Some(arguments) = function.get("arguments").or_else(|| delta.get("arguments")) {
            let part = arguments_text(Some(arguments));
            let prior = entry.get("arguments").and_then(Value::as_str).unwrap_or("").to_string();
            entry["arguments"] = Value::String(format!("{prior}{part}"));
            if entry.get("added").and_then(Value::as_bool).unwrap_or(false) { self.emit_tool_arguments(index, &part)?; }
        }
        Ok(())
    }
    ...
    fn ensure_tool_started(&mut self, index: usize) -> anyhow::Result<()> {
        ...
        let name_empty = self.tool_calls.get(&index).and_then(|e| e.get("name")).and_then(Value::as_str).unwrap_or("").is_empty();
        if already || name_empty { return Ok(()); }
        ...
        let item = json!({"type":"function_call","id":entry["item_id"],"call_id":entry["id"],"name":self.reverse_names.get(entry.get("name").and_then(Value::as_str).unwrap_or("")).cloned().unwrap_or_else(|| entry.get("name").and_then(Value::as_str).unwrap_or("tool").to_string()),"arguments":"","status":"in_progress"});
